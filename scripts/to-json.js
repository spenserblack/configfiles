...
async function main() {
  ...
  const configContents = await Promise.all(configFolders.map(async (folder) => {
    const configFiles = await readdir(resolve(configFilesPath, folder));
    const configData = await Promise.all(configFiles.map(async (file) => {
      const { name } = parse(file);
      const data = await readFile(resolve(configFilesPath, folder, file), 'utf-8');
      return { name, data };
    }));
  ...
  const configs = configContents.reduce((configs, { folder, configData }) => {
    const configDataObject = configData.reduce((configDataObject, { name, data }) => ({
      ...configDataObject,
      [name]: data,
    }), {});
